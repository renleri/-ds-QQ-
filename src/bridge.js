...
/** OneBot WS 客户端用的 token（wsAccessToken 优先，回退共享 accessToken）。 */
function oneBotWsToken(snowluma) {
  return snowluma?.wsAccessToken || snowluma?.accessToken || undefined;
}
...
import { createKnowledgeStore, buildKnowledgeContext, queryKnowledge, knowledgeStats, knowledgeIndex } from './knowledge-lib.js';
...
function sanitizeRoleName(name) {
  // 允许中日文、字母数字、连字符，以及书名号（角色卡名里会用，比如「鲸鱼娘《人》」）
...
function loadConfig() {
  ...
  const cfg = {
    ...
    snowluma: {
      ...
      accessToken: '',
      // SnowLuma 的 HTTP 与 WS 监听器各有独立 token；下面两个留空即回退到 accessToken。
      httpAccessToken: '',
      wsAccessToken: '',
      ...(file.snowluma ?? {})
    },
    ...
    // 只读旁观群：只学习（黑话 / 表情包），**永不发言、不投递 prompt、不排唤醒**。
    // 独立于 allow/deny，也不受聊天模式限制——因为它对 QQ 完全只读。
    observe: {
      groups: normalizeIdList(file.observe?.groups ?? []),
      learnSlang: file.observe?.learnSlang !== false,
      collectStickers: file.observe?.collectStickers !== false
    },
    // 「群主专属」群：群是放行的，但**只有管理员（ownerQQ）的消息会让她回应**。
    // 其他人的消息照样入库 / 学黑话 / 收表情，只是不触发唤醒（对该群等于只读旁观）。
    ownerOnly: {
      groups: normalizeIdList(file.ownerOnly?.groups ?? [])
    },
    // 作品知识库（knowledge/*.md）：永远注入一份目录（有哪些乐队/角色），
    // 对话里出现角色名或乐队名时再注入那一段详情——避免每轮都把整套设定烧进上下文。
    knowledge: {
      enabled: file.knowledge?.enabled !== false,
      dir: String(file.knowledge?.dir ?? 'knowledge'),
      maxChars: Math.max(400, Number(file.knowledge?.maxChars) || 2500),
      maxSections: Math.max(1, Number(file.knowledge?.maxSections) || 6),
      alwaysIndex: file.knowledge?.alwaysIndex !== false,
      historyLookback: Math.max(0, Number(file.knowledge?.historyLookback) ?? 12)
    },
    // 屏幕观察：主动机会时给**主人的私聊**附一张当前屏幕截图，让她有具体的话头。
    // onlyOwnerPrivate 默认 true —— 主人屏幕上的内容不该出现在群里。
    screen: {
      enabled: file.screen?.enabled !== false,
      onlyOwnerPrivate: file.screen?.onlyOwnerPrivate !== false,
      minIntervalMs: Math.max(30 * 1000, Number(file.screen?.minIntervalMs) || 5 * 60 * 1000),
      maxWidth: Math.max(320, Number(file.screen?.maxWidth) || 1100),
      quality: Math.min(95, Math.max(20, Number(file.screen?.quality) || 55)),
      // 归档目录：每张截图都留一份带时间戳的副本，方便主人查岗（空字符串 = 不归档）
      archiveDir: String(file.screen?.archiveDir ?? ''),
      archiveMaxFiles: Math.max(10, Number(file.screen?.archiveMaxFiles) || 500)
    },
    ...
    social: {
      enabled: true,
      // 角色卡注入上限（字符）。roles/小鲸鱼.md 有 7405 字符，原先写死 6000 会静默截断尾巴，
      // 而「表情包体系 / 轻量记忆 / 自我形象」这几节恰好落在截断点之后。
      roleMaxChars: 12000,
      // 启动阶段（观望）
    ...
    socialV2: {
      ...
      proactive: {
        ...
        probability: 0.3,
        // 对**主人的私聊**单独一套：更短间隔、更高概率，让她平时会主动来找主人说话。
        // 群聊仍走上面那套（在群里频繁主动冒泡不合适）。
        ownerPrivate: {
          enabled: true,
...
async function main() {
  ...
  // ── 作品知识库（knowledge/*.md）：目录常驻，详情按对话内容触发 ──────────
  // 用 store 而不是一次性读盘：改完 md 文件不用重启桥接，下一轮对话就生效。
  const knowledgeStore = createKnowledgeStore(path.resolve(ROOT, cfg.knowledge?.dir ?? 'knowledge'));
  {
    const stats = knowledgeStats(knowledgeStore.get());
    if (stats.bands > 0) log(`[knowledge] 已加载 ${stats.files} 份资料：${stats.bands} 个条目、${stats.members} 个角色`);
  }

  /**
   * 触发文本：本轮 prompt + 该会话最近若干条往来。
   * 只看 prompt 会漏掉「上一条提到香澄、这条只说『她』」的情况。
   */
  function knowledgeTriggerText(key, promptText) {
    const parts = [String(promptText ?? '')];
    const lookback = cfg.knowledge?.historyLookback ?? 12;
    if (key && lookback > 0) {
      try {
        const recent = getSocialV2State(key).recentMessages;
        if (Array.isArray(recent)) {
          for (const m of recent.slice(-lookback)) parts.push(String(m?.text ?? m?.content ?? ''));
        }
      } catch { /* 状态没准备好就只用 prompt */ }
    }
    return parts.join('\n');
  }

  function knowledgeBlock(key, promptText) {
    if (cfg.knowledge?.enabled === false) return '';
    try {
      const block = buildKnowledgeContext(knowledgeStore.get(), knowledgeTriggerText(key, promptText), {
        maxChars: cfg.knowledge?.maxChars ?? 2500,
        maxSections: cfg.knowledge?.maxSections ?? 6,
        alwaysIndex: cfg.knowledge?.alwaysIndex !== false
      });
      // 命中详情时记一行日志：出问题时能一眼看出「她到底有没有拿到资料」
      if (block.includes('【本轮提到的角色/乐队资料】')) {
        const names = block.split('【本轮提到的角色/乐队资料】')[1].split('\n').filter((l) => l.startsWith('### ')).map((l) => l.slice(4).trim());
        log(`[knowledge] 注入资料 → ${key ?? '未知会话'}：${names.join('、')}`);
      }
      return block;
    } catch (error) {
      log('[knowledge] 注入失败:', error?.message ?? error);
      return '';
    }
  }
  ...
  function withSlangContext(promptText, key = null) {
    const now = new Date();
    ...
    // 作品知识库：目录常驻，命中角色/乐队名时补详情（见 knowledge-lib.js 的设计说明）
    const knowledge = knowledgeBlock(key, promptText);
  ...
  // key -> 连续“繁忙”的起始时间：给 isConversationBusyV2 做统一时效兜底。
  // 只要会话连续繁忙超过 CONVERSATION_BUSY_LEASE_MS，就判定为陈旧状态并强制清理，
  // 避免漏收 turn/end、投递 promise 挂死等极端情况下该会话永久无法被唤醒（实测踩过一次）。
  const conversationBusySince = new Map();
  // 会话「繁忙」的统一时效租约（覆盖 pendingWakeTimer / pendingWakeKeys / promptQueue / turn 全部来源）。
  // 阈值需宽于最慢的真实回合（qq_wait_for_messages 单次最长 10 分钟，可能连续等待多次）。
  const CONVERSATION_BUSY_LEASE_MS = (() => {
    const configured = Number(cfg.socialV2?.busyLeaseMs);
  ...
  // ── 只读旁观群 ──────────────────────────────────────────────────────────
  // 目标：能看、能学（黑话 + 表情包），但对 QQ 完全只读——不投递 prompt、不排唤醒、不回话。
  // 实现在 handleIncoming 的最前面拦截，优先于白名单与模式判断。

  /** 该群号是否在旁观列表里。 */
  function isObservedGroup(id) {
    const list = cfg.observe?.groups ?? [];
    return list.map(String).includes(String(id));
  }

  /** 该群号是否在「群主专属」列表里（只有 ownerQQ 的消息会触发回应）。 */
  function isOwnerOnlyGroup(id) {
    const list = cfg.ownerOnly?.groups ?? [];
    return list.map(String).includes(String(id));
  }
   ...
   * 「群主专属」群的发送闸门：只回应主人。
   *
   * 光拦住「谁能唤醒她」是不够的——她一旦醒着，就会顺嘴接群友的话（实测踩过：主人在
   * 19:00:36 说完「是啊，吃什么」、19:01:32 群友接了句「吉野家」，她 19:01:47 就回了
   * 「这家行，起码米饭管够」）。所以发送这一层也要卡：主人最后一句之后如果群友又说话，
   * 一律不许发。返回错误文案=拦下；null=放行。
   */
  function ownerOnlyReplyBlock(key, kind, id) {
    if (kind !== 'group' || !isOwnerOnlyGroup(id)) return null;
    const ownerQQ = String(cfg.ownerQQ ?? '');
    if (!ownerQQ) return null;
    const recent = getSocialV2State(key).recentMessages;
    if (!Array.isArray(recent)) return null;
    for (let i = recent.length - 1; i >= 0; i--) {
      const m = recent[i];
      if (!m || m.isSelf) continue;
      const lastOther = String(m.userId ?? m.user_id ?? '');
      if (lastOther && lastOther !== ownerQQ) {
        log(`[reserved2] 拦截「群主专属」群的非主人回复 ${key}（最后发言者 ${lastOther}，主人 ${ownerQQ}）`);
        return '本群已设为「只回应主人」：主人的最后一句之后群友又说话了，这条不能发。等主人再开口。';
      }
      return null;
    }
    return null;
  ...
  /**
   * 旁观群里自动收藏表情包。
   * 复用 collectStickerV2（它按 messageRef 从会话消息库里查），所以调用方必须已经
   * appendSocialV2Message 过这条消息；频率限制沿用 socialV2.sticker.collect 的配置。
   */
  async function autoCollectStickerObserved(key, messageRef, mediaList, sender) {
    try {
      if (cfg.observe?.collectStickers === false) return;
      if (cfg.socialV2?.sticker?.enabled === false) return;
      if (cfg.socialV2?.sticker?.collect?.enabled === false) return;
      if (!messageRef) return;
      const media = (mediaList ?? []).find((m) => m && (m.kind === 'image' || m.kind === 'face'));
      if (!media) return; // 这条没有图片/表情
      const st = getSocialV2State(key);
      const now = Date.now();
      const perMinute = Number(cfg.socialV2?.sticker?.collect?.maxPerMinute) || 2;
      const perHour = Number(cfg.socialV2?.sticker?.collect?.maxPerHour) || 10;
      const times = Array.isArray(st.stickerCollectTimes) ? st.stickerCollectTimes : [];
      if (times.filter((t) => now - t < 60000).length >= perMinute) return;
      if (times.filter((t) => now - t < 3600000).length >= perHour) return;
      const result = await collectStickerV2(key, messageRef, '');
      st.stickerCollectTimes = [...times, now].slice(-500);
      saveSocialV2State();
      log(`[旁观] 收藏表情成功 ${key} ← ${sender}（emoji_id=${result.emojiId}）`);
    } catch (error) {
      log(`[旁观] 收藏表情失败 ${key}: ${error?.message ?? error}`);
    }
  }

  /** 处理一条来自旁观群的消息：入库 + 学黑话 + 收表情，然后直接结束（绝不回话）。 */
  async function handleObservedGroupMessage(key, event, sender, textContent, plainContent, mediaList, messageRef) {
    appendActivity(`${key} [旁观] ${sender}：${textContent.slice(0, 80)}`);
    // 先入库：既是收藏表情的查找来源，也让旁观上下文与正常会话隔离（旁观群永远不会被投递）
    try {
      appendSocialV2Message(key, sender, textContent, plainContent, false, false, messageRef || null, mediaList, event.user_id ?? null, extractForwardIds(event.message ?? []));
    } catch (error) {
      log(`[旁观] 消息入库失败 ${key}: ${error?.message ?? error}`);
    }
    if (cfg.observe?.learnSlang !== false) {
      try { feedSlangWindow(key, sender, plainContent); } catch (error) { log(`[旁观] 黑话入窗失败 ${key}: ${error?.message ?? error}`); }
    }
    // 收藏是异步的（要下载图片字节 + 调 OneBot），不阻塞消息接收
    void autoCollectStickerObserved(key, messageRef, mediaList, sender);
  }
      ...
      try {
        ...
        if (req.method === 'GET' && url.pathname === '/api/status') {
          ...
          sendJson({
            ...
            maxMessageChars: effectiveMaxMessageChars(),
            ownerPrivateLimits: {
              maxMessageChars: effectiveMaxMessageChars(`private:${String(cfg.ownerQQ ?? '')}`),
              burstMaxMessages: effectiveBurstMaxMessages(`private:${String(cfg.ownerQQ ?? '')}`),
            },
            roleDirectives: currentRoleDirectives(),
            duplicateGuard: {
              blocked: duplicateGuard.blocked,
              unverified: [...duplicateGuard.unverified.values()].reduce((n, l) => n + l.length, 0),
              sent: [...duplicateGuard.sent.values()].reduce((n, l) => n + l.length, 0),
            },
            dshReady,
        ...
        if (req.method === 'GET' && url.pathname === '/api/knowledge') {
          const kb = knowledgeStore.get();
          const q = url.searchParams.get('q');
          if (q) {
            const results = queryKnowledge(kb, q, 5);
            sendJson({ ok: true, query: q, count: results.length, results });
            return;
          }
          sendJson({
            ok: true,
            enabled: cfg.knowledge?.enabled !== false,
            dir: cfg.knowledge?.dir ?? 'knowledge',
            stats: knowledgeStats(kb),
            files: (kb.files ?? []).map((f) => ({ file: f.file, bands: f.bands })),
            index: knowledgeIndex(kb, { maxChars: 4000 })
          });
          return;
        }
        ...
        if (req.method === 'GET' && url.pathname === '/api/socialV2/prompt') {
          ...
          sendJson({
            ...
            recommended: cfg.socialV2?.provideRecommendations === false ? null : {
              ...
              // send 里报的是**当前角色实际生效**的上限：角色卡可用
              // <!-- @maxMessageChars: N --> 覆盖全局值，这里必须报真实值，否则她照推荐值发会被拒。
              send: { ...(cfg.socialV2?.send ?? {}), maxMessageChars: effectiveMaxMessageChars(key), burstMaxMessages: effectiveBurstMaxMessages(key) },
        ...
        if (req.method === 'POST' && url.pathname === '/api/socialV2/send-burst') {
          ...
          { const blocked = ownerOnlyReplyBlock(key, kind, id); if (blocked) { sendJson({ ok: false, error: blocked }, 403); return; } }
          const sendCfg = cfg.socialV2?.send ?? {};
          const maxMsgs = effectiveBurstMaxMessages(key);
          const maxChars = effectiveMaxMessageChars(key);
          if (maxMsgs > 0 && messages.length > maxMsgs) {
          ...
          {
            // 重复发送保护（qq_send_message / qq_reply 走的就是这个端点）：
            // 刚失败过（结果未知）或刚发成功过的完全相同内容，一律拦下。
            const dup = duplicateSendBlock(key, messages);
            if (dup) {
              log(`[dedupe] 拦下重复发送 ${key}：${String(messages[0]).slice(0, 40)}`);
              sendJson({ ok: false, error: dup }, 409);
              return;
            }
          }
          try {
        ...
        if (req.method === 'POST' && url.pathname === '/api/socialV2/send-message') {
          ...
          // 「群主专属」群：只回应主人（判定见 ownerOnlyReplyBlock）。
          {
            const blocked = ownerOnlyReplyBlock(key, kind, id);
            if (blocked) { sendJson({ ok: false, error: blocked }, 403); return; }
          }
          ...
          {
            // 重复发送保护（同统一发送端点）：结果未知的补发、以及刚发过的原文，一律拦下。
            const dup = duplicateSendBlock(key, messages);
            if (dup) {
              log(`[dedupe] 拦下重复发送 ${key}：${String(messages[0]).slice(0, 40)}`);
              sendJson({ ok: false, error: dup }, 409);
        ...
        if (req.method === 'POST' && (url.pathname === '/api/send/group' || url.pathname === '/api/send/private' || url.pathname === '/api/send/reply')) {
          ...
          const sendCfg = cfg.socialV2?.send ?? {};
          const maxChars = effectiveMaxMessageChars(key);
          if (maxChars > 0 && message.length > maxChars) { sendJson({ ok: false, error: `单条消息不能超过 ${maxChars} 字` }, 400); return; }
          if (SENSITIVE_RE.test(message)) { sendJson({ ok: false, error: '消息含敏感信息，已阻止发送' }, 403); return; }
          try {
            const st = getSocialV2State(key);
            const now = Date.now();
  ...
  /**
   * 读 state/ 目录下的本地图片（目前只有屏幕截图）。
   * 路径必须落在 STATE_DIR 内——否则这个分支就成了一条任意读文件的通道。
   */
  function readLocalMediaBuffer(p) {
    try {
      const base = path.resolve(STATE_DIR);
      const abs = path.resolve(String(p ?? ''));
      if (!abs.startsWith(base + path.sep)) return { buffer: null };
      const buf = fs.readFileSync(abs);
      if (!buf.length || buf.length > 8 * 1024 * 1024) return { buffer: null };
      return { buffer: buf, mimeType: /\.png$/i.test(abs) ? 'image/png' : 'image/jpeg' };
    } catch {
      return { buffer: null };
    }
  }

  // ── 屏幕观察：让她在「主动找主人说话」时能看到主人此刻在干什么 ──────────
  // 约束（重要）：
  //   - 默认**只允许主人的私聊**，群聊一律不截——主人在屏幕上的东西不该出现在群里；
  //   - 有最小间隔，避免每次唤醒都截；
  //   - 截图文件固定落在 state/，由 readLocalMediaBuffer 白名单校验；
  //   - 一句话提示里明确写了「私密内容不许说出口、不许转发」。
  let lastScreenCaptureAt = 0;
  let screenCaptureBusy = false;

  function screenAllowedFor(key) {
    const s = cfg.screen ?? {};
    if (s.enabled === false) return false;
    if (s.onlyOwnerPrivate !== false) return key === `private:${String(cfg.ownerQQ ?? '')}`;
    return true;
  }

  /** 截一张主人的屏幕，成功则返回可直接塞进 prompt 的 media 对象；否则 null。 */
  async function captureScreen(key) {
    if (!screenAllowedFor(key)) return null;
    const s = cfg.screen ?? {};
    const minGap = Math.max(30 * 1000, Number(s.minIntervalMs) || 5 * 60 * 1000);
    if (screenCaptureBusy || Date.now() - lastScreenCaptureAt < minGap) return null;
    const script = path.join(ROOT, 'scripts', 'capture-screen.ps1');
    if (!fs.existsSync(script)) return null;
    const out = path.join(STATE_DIR, 'screen.jpg');
    const maxWidth = Math.max(320, Number(s.maxWidth) || 1100);
    const quality = Math.min(95, Math.max(20, Number(s.quality) || 55));
    screenCaptureBusy = true;
    try {
      await new Promise((resolve, reject) => {
        const child = spawn('powershell', [
          '-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass', '-File', script,
          '-Out', out, '-MaxWidth', String(maxWidth), '-Quality', String(quality),
        ...
        child.on('exit', (code) => { clearTimeout(timer); code === 0 ? resolve() : reject(new Error(`截图进程退出码 ${code}`)); });
      });
      if (!fs.existsSync(out)) return null;
      lastScreenCaptureAt = Date.now();
      const bytes = fs.statSync(out).size;
      const sizeKb = Math.round(bytes / 1024);
      // 存档：每次截图都往「查岗记录」目录里留一份带时间戳的副本，方便主人翻看。
      // 归档失败不影响本次唤醒（截图已经拿到了）。
      archiveScreenShot(out, key, sizeKb);
      log(`[screen] 已截取屏幕 ${sizeKb}KB → ${key}`);
      // 锁屏/息屏时截出来几乎全黑，文件也会异常小（实测 11KB vs 正常 30~100KB）。
      // 这种就别塞给她了——对着黑屏她只能瞎猜；存档照留，方便主人核对。
      const minUseful = Math.max(1000, Number(cfg.screen?.minUsefulBytes) || 15000);
      if (bytes < minUseful) {
        log(`[screen] 这张只有 ${sizeKb}KB，疑似锁屏/黑屏，本次不附给她（存档仍保留）`);
        return null;
      }
      return { kind: 'local', path: out, caption: '【主人此刻的屏幕（截图）】' };
    } catch (error) {
      log(`[screen] 截图失败：${error?.message ?? error}`);
      return null;
    } finally {
  ...
  function archiveScreenShot(srcPath, key, sizeKb) {
    // 注意：这里读的是 cfg.screen.archiveDir，配置加载时必须把该字段带过来，
    // 否则会静默不归档（第 26 轮就踩过：截图成功、归档一声不响）。
    const dir = String(cfg.screen?.archiveDir ?? '').trim();
    if (!dir) return;
    try {
      fs.mkdirSync(dir, { recursive: true });
      const d = new Date();
      ...
      const safeKey = String(key ?? 'unknown').replace(/[^\w-]/g, '-');
      const name = `${stamp}_${safeKey}.jpg`;
      const dest = path.join(dir, name);
      fs.copyFileSync(srcPath, dest);
      const line = `${d.toLocaleString('zh-CN', { hour12: false })}\t${key}\t${name}\t${sizeKb}KB\n`;
      fs.appendFileSync(path.join(dir, 'index.tsv'), line, 'utf8');
      // 超量清理：只保留最近 archiveMaxFiles 张（按文件名排序 = 按时间排序）
      const maxFiles = Math.max(10, Number(cfg.screen?.archiveMaxFiles) || 500);
      const shots = fs.readdirSync(dir).filter((f) => /\.jpe?g$/i.test(f)).sort();
      let removed = 0;
      while (shots.length - removed > maxFiles) {
        try { fs.unlinkSync(path.join(dir, shots[removed])); removed++; } catch { break; }
      }
      fs.appendFileSync(path.join(dir, 'index.tsv'), '', 'utf8');
      log(`[screen] 已归档 → ${dest}${removed ? `（顺带清理了 ${removed} 张旧图）` : ''}`);
    } catch (error) {
      log(`[screen] 归档失败（不影响本次唤醒）：${error?.message ?? error}`);
    }
  }
  ...
  async function resolveMediaList(mediaList) {
    ...
    for (const media of Array.isArray(mediaList) ? mediaList : []) {
      ...
      if (media.kind === 'face') {
        ...
        // 本地图片（目前只有屏幕截图）。路径必须落在 state/ 目录内，
        // 免得这个分支变成一条任意读文件的通道。
        const local = readLocalMediaBuffer(media.path);
        if (local.buffer) {
          if (totalBytes + local.buffer.length > MAX_MEDIA_BYTES) {
            parts.push({ type: 'text', text: `[图片${index}（图片总大小超限，已跳过）]` });
            continue;
          }
          totalBytes += local.buffer.length;
          if (media.caption) parts.push({ type: 'text', text: String(media.caption) });
          parts.push({ type: 'image', mediaType: local.mimeType, data: local.buffer.toString('base64'), name: `local-${path.basename(String(media.path))}` });
        } else {
          parts.push({ type: 'text', text: `[图片${index}（本地文件不可读）]` });
        }
      } else {
        const img = await fetchOneBotImage(media);
        if (img?.buffer) {
  ...
  // ── 重复发送保护（第 23 轮） ─────────────────────────────────────────────
  // 起因：OneBot 对一条**其实已经发出去**的消息回了「网络连接异常!」，桥接把失败如实转给 AI，
  // AI 以为没发出去就补了一条 → 主人收到两条一样的。
  // 思路：把「结果未知」的失败记下来；短时间内再发同样内容就拦掉，并让它先去翻聊天记录确认。
  // 另外对「刚发成功过的完全相同内容」也做一次短窗口拦截，堵住其它可能的重复路径。
  const UNVERIFIED_SEND_RE = /网络连接异常|timed?\s?out|ETIMEDOUT|socket hang up|fetch failed|aborted|network/i;
  const duplicateGuard = {
    unverified: new Map(), // key -> [{ text, ts }]
    sent: new Map(),       // key -> [{ text, ts }]
    blocked: 0,
  };

  function pushGuardEntry(map, key, text, windowMs) {
    const now = Date.now();
    const list = (map.get(key) ?? []).filter((e) => now - e.ts < windowMs);
    list.push({ text: String(text), ts: now });
    map.set(key, list.slice(-20));
  }

  function guardWindow(name) {
    const g = cfg.socialV2?.send?.duplicateGuard ?? {};
    if (g.enabled === false) return 0;
    return name === 'unverified'
      ? Math.max(0, Number(g.unverifiedWindowMs) || 180000)
      : Math.max(0, Number(g.windowMs) ?? 60000);
  }

  function rememberUnverifiedSend(key, text) {
    pushGuardEntry(duplicateGuard.unverified, key, text, guardWindow('unverified') || 180000);
  }
  function rememberSentText(key, text) {
    pushGuardEntry(duplicateGuard.sent, key, text, guardWindow('sent') || 60000);
  }

  /**
   * 这批要发的消息里，有没有「刚失败过（结果未知）」或「刚发成功过」的完全相同的文本？
   * 返回提示文案表示应拦下，null 表示放行。
   */
  function duplicateSendBlock(key, messages) {
    const now = Date.now();
    const unvWin = guardWindow('unverified');
    const okWin = guardWindow('sent');
    const list = (map, win) => (win > 0 ? (map.get(key) ?? []).filter((e) => now - e.ts < win) : []);
    const unv = list(duplicateGuard.unverified, unvWin);
    const ok = list(duplicateGuard.sent, okWin);
    for (const raw of messages) {
      const text = String(raw ?? '');
      if (!text) continue;
      if (unv.some((e) => e.text === text)) {
        duplicateGuard.blocked++;
        return `这条刚才发送时报过网络错误、**结果未知**（很可能已经发出去了），不要再补发。请先调用 qq_get_recent_messages 翻一下聊天记录，确认没发出去再说；确认真没发出去，可以改一两个字再发。`;
      }
      if (ok.some((e) => e.text === text)) {
        duplicateGuard.blocked++;
        return `这条和刚刚发成功的内容一模一样，重复发送会被拦下（真人不会连发两条同样的）。想强调就换个说法。`;
      }
    }
    return null;
  }
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      sendChain = sendChain
        .then(async () => {
          ...
          sent.push(msg);
          rememberSentText(key, msg);
        })
        .catch((error) => {
          log(`QQ 发送失败 (${key}):`, error?.message ?? error);
          // 网络类错误 = 结果未知：消息可能已经出去了。记下来，短窗口内不允许原样补发。
          if (UNVERIFIED_SEND_RE.test(String(error?.message ?? error))) {
            rememberUnverifiedSend(key, msg);
            log(`[dedupe] 该条结果未知，已登记为「可能已发出」(${key})：${String(msg).slice(0, 40)}`);
          }
          failed.push(error);
        });
      if (i < delays.length) {
  ...
  async function deliverPromptNow(key, promptText, opts = {}) {
    ...
    let content = [{ type: 'text', text: withSlangContext(promptText, key) }];
    if (Array.isArray(opts.media) && opts.media.length > 0) {
      const imageParts = await resolveMediaList(opts.media);
      content = [{ type: 'text', text: withSlangContext(promptText, key) }, ...imageParts];
    }
  ...
  // 角色卡里的机器可读指令：<!-- @maxMessageChars: 20 -->
  // 这类注释不会被注入 prompt（否则白占上下文、还可能被她念出来），只用来给桥接读配置。
  // 好处是「这一份角色卡用什么说话风格」写在同一份文件里，不跟全局配置打架。
  const ROLE_DIRECTIVE_RE = /^[ \t]*<!--\s*@([A-Za-z][\w]*)\s*[:=]\s*([^>]*?)\s*-->[ \t]*$/gm;

  function parseRoleDirectives(text) {
    const out = {};
    for (const m of String(text ?? '').matchAll(ROLE_DIRECTIVE_RE)) out[m[1]] = m[2].trim();
    return out;
  }

  function stripRoleDirectives(text) {
    return String(text ?? '').replace(ROLE_DIRECTIVE_RE, '').replace(/\n{3,}/g, '\n\n').trim();
  }
  ...
  let roleHintCache = { key: '', hint: '', directives: {} };
  function currentRoleHint() {
    try {
      ...
      const directives = parseRoleDirectives(hint);
      hint = stripRoleDirectives(hint);
      const roleMaxChars = Number(cfg.social?.roleMaxChars) > 0 ? Number(cfg.social.roleMaxChars) : 6000;
      if (hint.length > roleMaxChars) hint = hint.slice(0, roleMaxChars);
      roleHintCache = { key: cacheKey, hint, directives };
      return hint;
    } catch {
      return '';
    }
  ...
  /** 当前角色卡声明的指令（可能为空；读不到角色就返回空对象）。 */
  function currentRoleDirectives() {
    currentRoleHint(); // 触发一次读取/缓存刷新
    return roleHintCache.directives ?? {};
  }

  /** 这个会话是不是「主人的私聊」——她在这里不受字数/条数限制。 */
  function isOwnerPrivateKey(key) {
    return !!key && key === `private:${String(cfg.ownerQQ ?? '')}`;
  }

  /**
   * 单条消息字数上限。
   * 优先级：主人的私聊（`send.ownerPrivate.maxMessageChars`，0 = 不限）
   *        → 角色卡的 <!-- @maxMessageChars: N -->（这份角色卡自己的风格）
   *        → 全局 `send.maxMessageChars`。
   * 返回 0 表示不限长度。
   */
  function effectiveMaxMessageChars(key = null) {
    if (isOwnerPrivateKey(key)) {
      const v = cfg.socialV2?.send?.ownerPrivate?.maxMessageChars;
      const n = v === null || v === undefined ? 0 : Number(v);
      if (Number.isFinite(n) && n >= 0) return Math.round(n);
    }
    const n = Number(currentRoleDirectives().maxMessageChars);
    if (Number.isFinite(n) && n > 0) return Math.round(n);
    const fallback = Number(cfg.socialV2?.send?.maxMessageChars);
    return Number.isFinite(fallback) && fallback > 0 ? Math.round(fallback) : 500;
  }

  /** 一次最多分几条：主人的私聊可单独放宽（0 = 不限）。 */
  function effectiveBurstMaxMessages(key = null) {
    if (isOwnerPrivateKey(key)) {
      const v = cfg.socialV2?.send?.ownerPrivate?.burstMaxMessages;
      const n = v === null || v === undefined ? 0 : Number(v);
      if (Number.isFinite(n) && n >= 0) return Math.round(n);
    }
    const n = Number(cfg.socialV2?.send?.burstMaxMessages);
  ...
  function evaluateWakeTriggerV2(key, st, event, kind, textContent, plainContent, quoteTargetIsSelf) {
    ...
    // 主人自己在群里说话，一律叫醒她（默认开）。
    // 起因：群里的普通消息只有 5% 概率唤醒，于是主人@不动她、也没关键词时，
    // 他发的消息会「看起来像她不理人」；但主人开口本来就该被看见。
    // 想恢复成完全按概率，把 wake.ownerMessagesAlwaysWake 设成 false。
    if (cfg.socialV2?.wake?.ownerMessagesAlwaysWake !== false) {
      const speakerId = String(event?.user_id ?? event?.sender?.user_id ?? '');
      const ownerId = String(cfg.ownerQQ ?? '');
      if (ownerId && speakerId === ownerId) return 'owner';
    }
  ...
  function buildWakePromptV2(key, reason) {
    ...
    if (reason === 'proactiveCheck') {
      // 主人的私聊：这是「她主动来找主人」的主要入口，提示要写得具体，别让每次都是「在吗」。
      if (key === `private:${String(cfg.ownerQQ ?? '')}`) {
        return `${base}【主动机会】${key}\n原因：你和主人已经有一段时间没说话了，这是一次你可以主动找他聊的机会。\n做法：先 qq_memory_query 看看有没有「想说还没说的话」或没聊完的话题；没有就自己想一个具体的由头——问他吃了没（你最关心米饭）、说说你刚想到的事、吐槽点什么、或者把上次的话题捡起来。\n别每次都「在吗」「干嘛呢」，那样很干；宁可说一件具体的小事。发 1~2 条、每条不超过 20 字，然后用 qq_wait_for_messages 等他回。\n如果实在不想说话，再安静收尾（qq_mark_read 或 qq_set_wake_config）。`;
      }
  ...
  async function sendWakePromptV2(key, reason) {
    ...
    try {
      ...
      const result = await deliverPrompt(key, finalPromptText, wakeMedia.length ? { media: wakeMedia } : {});
  ...
  // 会话「繁忙」的统一时效租约：pendingWakeKeys 原本有 30 分钟租约，但另外几个来源
  // （pendingWakeTimer / promptQueues.running / v2TurnStartAt / collectors）都没有兜底：
  // 一旦漏收 turn/end 或某个投递 promise 挂死，该会话就会**永久**被判为繁忙，
  // 之后所有 @/提问只被「暂存」，再也不会回话。这里改成按「连续繁忙时长」统一兜底。
  // 常量 CONVERSATION_BUSY_LEASE_MS 在文件上方与其它状态 Map 一起声明。
  function isConversationBusyV2(key, st) {
    const pendingTimer = Boolean(st && st.pendingWakeTimer);
    const wakeLeased = pendingWakeKeys.has(key);
    const q = promptQueues.get(key);
    const queueBusy = Boolean(q && (q.running || q.queue.length > 0));
    const sid = state.sessions[key];
    const turnBusy = Boolean(sid && (v2TurnStartAt.has(sid) || collectors.has(sid)));

    if (!pendingTimer && !wakeLeased && !queueBusy && !turnBusy) {
      conversationBusySince.delete(key); // 空闲时清零，下次繁忙重新计时
      return false;
    }

    const now = Date.now();
    const since = conversationBusySince.get(key);
    if (since === undefined) {
      conversationBusySince.set(key, now);
      return true;
    }
    if (now - since <= CONVERSATION_BUSY_LEASE_MS) return true;

    // —— 连续繁忙超时：判定为陈旧状态，清掉全部可疑标记并放行这次唤醒 ——
    log(`[reserved2] ${key} 连续繁忙 ${Math.round((now - since) / 60000)} 分钟，判定为陈旧状态并强制解除`
      + `（来源：${[pendingTimer && 'pendingWakeTimer', wakeLeased && 'pendingWakeKeys', queueBusy && 'promptQueue', turnBusy && 'turn'].filter(Boolean).join('+')}）`);
    if (pendingTimer) {
      clearTimeout(st.pendingWakeTimer);
      st.pendingWakeTimer = null;
    }
    ...
    conversationBusySince.delete(key);
    return false;
  }
  ...
  /**
   * 主动机会的实际参数：主人的私聊走 proactive.ownerPrivate（更积极），其余走 proactive。
   * 之所以按会话分两套：主人希望她平时会主动来找自己说话，但在群里频繁主动冒泡并不合适。
   *
   * 另外两个「让她别太容易被压下去」的开关：
   * - recentReplyDampenMs：刚说过话的冷却期（原先是写死的 30 分钟，对她来说太长——
   *   她只要回过一句，之后半小时的概率都砍半，于是主动机会被自己压没了）；
   * - longSilenceMs / longSilenceProbability：主人很久没说话时，给一次「无论如何都去看看」的机会。
   */
  function proactiveParams(key) {
    const p = cfg.socialV2?.proactive ?? {};
    const isOwnerPrivate = key === `private:${String(cfg.ownerQQ ?? '')}`;
    const o = p.ownerPrivate ?? {};
    const pick = (a, b, dflt) => {
      const v = Number(a);
    ...
    if (isOwnerPrivate && o.enabled !== false) {
      return {
        min: pick(o.checkIntervalMinMs, p.checkIntervalMinMs, 10 * 60 * 1000),
        max: pick(o.checkIntervalMaxMs, p.checkIntervalMaxMs, 25 * 60 * 1000),
        idle: pick(o.idleThresholdMs, p.idleThresholdMs, 5 * 60 * 1000),
        prob: Number.isFinite(Number(o.probability)) ? Number(o.probability) : 0.8,
        dampenMs: pick(o.recentReplyDampenMs, undefined, 10 * 60 * 1000),
        longSilenceMs: pick(o.longSilenceMs, undefined, 2 * 60 * 60 * 1000),
        longSilenceProb: Number.isFinite(Number(o.longSilenceProbability)) ? Number(o.longSilenceProbability) : 1,
      };
    }
    return {
      min: pick(p.checkIntervalMinMs, undefined, 30 * 60 * 1000),
      max: pick(p.checkIntervalMaxMs, undefined, 90 * 60 * 1000),
      idle: pick(p.idleThresholdMs, undefined, 15 * 60 * 1000),
      prob: Number.isFinite(Number(p.probability)) ? Number(p.probability) : 0.3,
      dampenMs: pick(p.recentReplyDampenMs, undefined, 30 * 60 * 1000),
      longSilenceMs: pick(p.longSilenceMs, undefined, 0),
      longSilenceProb: Number.isFinite(Number(p.longSilenceProbability)) ? Number(p.longSilenceProbability) : 0,
    };
  }
  ...
  function scheduleProactiveCheckV2(key) {
    ...
    st.proactiveTimer = setTimeout(() => {
      ...
      if (st.lastAiReplyAt && Date.now() - Number(st.lastAiReplyAt) < p.dampenMs) prob *= 0.5;
      ...
      // 长时间没人说话：这次不再掷骰，直接给她一次机会（否则主人不开口，她也永远不开口）
      const longSilent = p.longSilenceMs > 0 && idle >= p.longSilenceMs;
      if (longSilent) prob = Math.max(prob, p.longSilenceProb);
  ...
  async function handleIncoming(kind, id, event, cfg) {
    ...
    // 旁观群优先于白名单/模式：它没有被 allow 也要处理（但只学不回）
    const observed = kind === 'group' && isObservedGroup(id);
    if (!observed && !modeAllowed(key, kind, id, cfg, currentMode)) {
    ...
    // 旁观群：入库 + 学黑话 + 收表情，然后立刻结束——不投递 prompt、不排唤醒、不回话。
    // 必须放在下面的「空文本过滤」之前：表情包消息经常只有图片没有文字，
    // 否则会在这里被 `if (!plainContent && !quoteTargetIsSelf) return;` 直接丢掉。
    if (observed) {
      await handleObservedGroupMessage(
        key,
        event,
    ...
    // 「群主专属」群：只对管理员的消息有反应。其他人的消息仍然入库 + 学黑话 + 收表情，
    // 但绝不投递 prompt、不排唤醒（等价于该群对别人是只读旁观，对管理员是正常群聊）。
    // 必须放在下面的空文本过滤之前，否则别人发的纯图片消息不会被记录/收藏。
    if (kind === 'group' && !isOwner && isOwnerOnlyGroup(id)) {
      await handleObservedGroupMessage(key, event, senderName, textContent, plainContent, mediaList, messageRef);
      return;
    }
    ...
    // 自然语言角色控制：roles/README.md 教的就是「进入角色扮演：<名字>」，
    // 但过去只实现了「非管理员拦截」，管理员说这句会原样转发给模型（因此群里发它等于没用）。
    // 非管理员在上方已 return，走到这里说明是管理员；行为与 /role 完全一致。
    {
      const switchMatch = /(?:进入角色扮演|切换角色|设置角色|改角色|换角色|开启角色扮演)\s*[:：]?\s*(.+)$/u.exec(plainContent);
      const wantsExit = /退出角色扮演|关闭角色扮演/u.test(plainContent);
      if (switchMatch || wantsExit) {
        const name = switchMatch ? sanitizeRoleName(switchMatch[1].trim()) : '';
        if (!name) {
          writeRoleState(null, roleState.mode);
          await sendToQQ(key, '已清除角色，恢复正常人格。');
        } else if (!fs.existsSync(path.join(ROOT, 'roles', name + '.md'))) {
          await sendToQQ(key, `角色「${name}」不存在。现有角色：${listRoles().join('、') || '（无）'}。`);
        } else {
      ...
      try {
        ...
        for await (const envelope of api.events.mux({})) {
          ...
          if (frame.type === 'session/event') {
            ...
            // 半途接入事件流（例如桥接在某个回合进行中重启）时，可能只收到 turn/end 而没有 turn/start：
            // 此时 createTurnCollector 里没有该 turn 的记录，push 返回 null，但回合确实已经结束。
            // 必须在 `ended` 之外单独兜底清理，否则 collectors 会留下永久条目，
            // 使该会话被 isConversationBusyV2 永久判为「繁忙」——之后所有 @/提问只被“暂存”，永远不回话。
            if (!ended && frame.event.type === 'turn/end') {
            ...
            if (ended) {
              ...
              if (ended.reason.kind === 'completed' && ended.text.trim()) {
                ...
                // 报错要不要在聊天里回一句？默认**不回**（主人要求：报错不必发到 QQ）。
                // 改成只记日志 + 控制台活动流，这样出问题仍然查得到，但不会拿 API 报错刷屏。
                // 想恢复原行为：socialV2.feedback.notifyChatOnError = true。
                const notifyChat = cfg.socialV2?.feedback?.notifyChatOnError === true;
                if (notifyChat) {
                  await sendToQQ(key, `⚠️ agent 处理出错：${safeMsg}`);
                }
                log(`agent 处理出错（${notifyChat ? '已发到 QQ' : '仅记录，不发 QQ'}）(${key})：${safeMsg}`);
                appendActivity(`${key} agent 处理出错：${safeMsg}`);
                if (isFarewell) {
                  const st = socialState(key);
                  if (st.phase === 'exiting') {
                    st.phase = 'idle';
  ...
  const bot = new SnowLumaWebSocketClient({
    ...
    accessToken: oneBotWsToken(cfg.snowluma),
    reconnect: true
  });
